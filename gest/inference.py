...
class CvCameraInferencePipeline(Pipeline):
    ...
    def video_capture(self, items):
        ...
        logger.info(f'[전] WIDTH 값 {capture.get(cv2.CAP_PROP_FRAME_WIDTH)}  [전] WIDTH 값 {capture.get(cv2.CAP_PROP_FRAME_HEIGHT)} ')
        #capture.set(cv2.CAP_PROP_FRAME_WIDTH, IMAGE_WIDTH)
        #capture.set(cv2.CAP_PROP_FRAME_HEIGHT, IMAGE_HEIGHT)
        logger.info(f'[후] WIDTH 값 {capture.get(cv2.CAP_PROP_FRAME_WIDTH)}  [후] HEIGHT 값 {capture.get(cv2.CAP_PROP_FRAME_HEIGHT)} ')
        for item in items:
